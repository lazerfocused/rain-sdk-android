...
object Erc20Abi {
    ...
    /**
     * Encodes `transfer(address,uint256)` calldata for a decimal [amount] of a token
     * with [decimals], using the same base-unit conversion the adapters used inline.
     */
    fun encodeTransfer(toAddress: String, amount: BigDecimal, decimals: Int): String =
        encodeTransfer(toAddress, amount.multiply(BigDecimal.TEN.pow(decimals)).toBigInteger())
}
