...
class TransactionCoordinatorTest {
  ...
  @Before
  fun setUp() {
    walletProvider = mockk(moreInterfaces = arrayOf(RainTransactionFeeEstimatingProvider::class))
    validator = mockk()
    signer = mockk()
    executor = mockk()
  ...
  fun `estimateGas delegates to walletProvider estimateTransactionFee`() = runBlocking {
    ...
    coEvery {
      (walletProvider as RainTransactionFeeEstimatingProvider)
        .estimateTransactionFee(chainId, from, to, data, "0x0")
    } returns expectedFee
