 ...
 *
 * Maps Turnkey SDK, Web3j, and other third-party errors to standardized [RainError] types.
 * Provides consistent error detection across the SDK. Part of the adapter construction kit:
 * `public` so sibling provider modules can map their own vendor errors at the boundary, but
 * `@RestrictTo(LIBRARY_GROUP)` so app code can't.
 */
