...
interface RainClient {
    ...
    /**
     * Registers a [WalletProvider] and makes it the active provider. Mirrors the iOS
     * `register(_:)`. Adapters self-register from the app (DI-friendly); a single-provider
     * app is just the N=1 case. Re-registering the same [ProviderId] replaces it.
     *
     * @param provider The provider to register and activate.
     */
    fun register(provider: WalletProvider)

    /**
     * Returns the registered provider with the given [id].
     *
     * @throws RainError.WalletUnavailable if no provider is registered under [id].
     */
    @Throws(RainError::class)
    fun provider(id: ProviderId): WalletProvider

    /**
     * Returns all registered providers that advertise [capability], in registration order.
     */
