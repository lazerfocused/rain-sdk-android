 ...
 * - Turnkey wallet integration (baseline; `client.initializeTurnkey(...)`)
 * - Wallet-agnostic mode + custom providers (`client.initialize(...)` + `register`/`setWalletProvider`)
 * - Transaction builder utilities
 *
 * Portal lives in the separate `:rain-portal` module — add that dependency and use
 * `com.rain.sdk.portal.initializePortal` / `PortalProvider`.
 *
 * Usage:
 * ```kotlin
 * // Turnkey
 * RainSdk.getInstance().client.initializeTurnkey(turnkey, rpcEndpoints)
 *
